...
#ifndef VIEWS_GAME_JUMPPOINTSEARCH_JPS_GRID_C_
#define VIEWS_GAME_JUMPPOINTSEARCH_JPS_GRID_C_
...
struct node **_buildNodes( int width, int height, bool **matrix)
{
	int i, j;
	size_t malloc_param;
	int size_node_pointer=sizeof(struct node *), size_node_pointer_pointer=sizeof(struct node**);
	struct node **nodes;
	malloc_param=(size_t)(height * size_node_pointer);
	nodes = (struct node **) malloc(malloc_param);
	/*malloc_count++;*/ /* [ Malloc Count ] */

	for (i = 0; i < height; i++) {
		malloc_param=(size_t)(width * size_node_pointer_pointer);
		nodes[i] = (struct node *) malloc(malloc_param);
		/*malloc_count++;*/ /* [ Malloc Count ] */
		for (j = 0; j < width; ++j) {
...
#endif /* VIEWS_GAME_JUMPPOINTSEARCH_JPS_GRID_C_ */
